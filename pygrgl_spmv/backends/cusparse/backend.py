...
def _selector_bytes_from_scan(scan: ArtifactScan) -> int:
    ...
    total = 0
    for level, (mut_nnz, miss_nnz) in enumerate(scan.selector_nnz_by_level):
...
def _metadata_bytes_from_scan(scan: ArtifactScan, pair: CusparsePlanPair, requirements: RuntimeRequirements, dtype: np.dtype) -> int:
    ...
    total = int(scan.num_nodes * scan.node_perm_dtype.itemsize)
    total += int(scan.num_samples * scan.sample_to_individual_dtype.itemsize)
