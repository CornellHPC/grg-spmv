...
def test_missing_nodes_and_counts(missing_grg):
    total_miss_idv = 0
    total_miss = 0
    for _mut_id, node, miss in missing_grg.get_mutation_node_miss():
...
def test_missing_matmul_semantics(missing_grg, missing_artifact):
    with _open_reference_grg(missing_artifact) as runtime:
        ...
        ref_up = np.asarray(pygrgl.matmul(missing_grg, rv_up, pygrgl.TraversalDirection.UP, by_individual=True, miss=miss_ref))
        miss_op = np.zeros((rows, missing_grg.num_mutations), dtype=np.float64)
        got_up = grg.matmul(rv_up, pygrgl.TraversalDirection.UP, by_individual=True, miss=miss_op)
        ...
        ref_down = np.asarray(pygrgl.matmul(missing_grg, rv_down, pygrgl.TraversalDirection.DOWN, by_individual=True, miss=miss_in.copy()))
        got_down = grg.matmul(rv_down, pygrgl.TraversalDirection.DOWN, by_individual=True, miss=miss_in.copy())
...
def test_shared_site_missingness_affects_all_variants(missing_grg, missing_artifact):
    with _open_reference_grg(missing_artifact) as runtime:
        ...
        miss_ref = np.zeros((rows, missing_grg.num_mutations), dtype=np.float64)
        _ = pygrgl.matmul(missing_grg, rv, pygrgl.TraversalDirection.UP, by_individual=True, miss=miss_ref)
        miss_op = np.zeros((rows, missing_grg.num_mutations), dtype=np.float64)
        _ = grg.matmul(rv, pygrgl.TraversalDirection.UP, by_individual=True, miss=miss_op)
        np.testing.assert_allclose(miss_op, miss_ref)
